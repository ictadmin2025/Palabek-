		...
		if ( $order != null ) {
			...
			if ( $orderby_meta == null ) {
				...
				if ( in_array( $orderby, [ 'etn_start_date', 'etn_end_date' ] ) ) {
					...
					add_filter( 'posts_orderby', function ( $orderby_statement, $query ) {
						global $wpdb;
						if ( $query->get( 'orderby' ) && is_array( $query->get( 'orderby' ) ) ) {
							return "
								...
								(SELECT meta_value FROM {$wpdb->postmeta} WHERE post_id = {$wpdb->posts}.ID AND meta_key = 'etn_start_time') {$query->get('order')}
							";
						}
 ...
 */
	 ...
	 */
	public static function get_tickets_by_event( $current_post_id, $report_sorting = "DESC" ) {
		global $wpdb;
		$response_data = [];
		$data          = [];
	...
	public static function get_event_location() {
		...
		if ( ! empty( $location_query_data ) ) {
			foreach ( $location_query_data as $value ) {
				...
				if ( ('offline' === $event_type || 'hybrid' === $event_type) && is_array($location) && isset($location['address']) && !empty($location['address']) && is_string($location['address']) ) {
	...
	public static function generate_attendee_ticket_email_markup( $attendee_id ) {
		...
		?>
		<?php if ( ('online' === $event_type || 'hybrid' === $event_type) && $meeting_link ): ?>
        ...
        <div class="etn-attendee-details-button-parent">
            ...
            <div class="etn-attendee-details-button-download">
                ...
                <span class="etn-attendee-details-separator"> | </span>
	...
	public static function process_attendee_ticket_email( $current_order_attendees, $event_name, $pdf_data, $order_status ) {
		...
		if ( ( $send_email_to_attendees ) && is_array( $pdf_data ) && ! empty( $pdf_data['user_email'] ) ) {
